...
class EmployeesController extends AdminBaseController
{
    ...
    public function ajax_employees()
    {
        $result = Employee::manager(admin()->id)
            ->select('employees.id','shifts.shift_name as shifts','shifts.time_in as time_in','shifts.time_out as time_out', 'employees.employeeID as employeeID', 'employees.profile_image', 'employees.full_name', 'department.name', 'designation.designation', DB::raw('1 as work'), 'employees.status', 'employees.created_at')
            ->get();
...
//
    ...
    public function create()
    {
        ...
        $this->data["devices"] = CompanyDevices::select('company_devices.id as id', 'company_device_id')
                ->where('company_id',$this->company_id)
                ->pluck('company_device_id', 'company_devices.id');

        $this->data["shifts"] = Shifts::select('shifts.id as id', 'shift_name')
                ->where('company_id',$this->company_id)
                ->pluck('shift_name', 'shifts.id');
    ...
    public function update(UpdateRequest $request, $id)
    {
        ...
        if ($request->updateType == 'shiftManagment') {
            $companyDetails = $employee;
            $companyDetails->shift_id = $request->shift_id;
            $companyDetails->save();
            return Reply::success('Shift Details updated successfully');
        }


        if ($request->updateType == 'deviceSetting') {
            $companyDetails = $employee;

            $companyDetails->registered_device = $request->registered_device;
            $companyDetails->device_user_is =  $request->device_user_is;
            $companyDetails->save();
          

    

            return Reply::success('Device Details updated successfully');
