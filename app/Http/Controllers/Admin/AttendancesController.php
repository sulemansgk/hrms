    ...
    public function ajax_attendance(Request $request)
    {
        ...
        $result = Employee::manager(admin()->id)
            ...
            ->select('employees.full_name as full_name',
                ...
                'shifts.time_in as timeIn',
                'shifts.time_out as timeOut',
            )
            ->where("employees.status", "active")
            ->get();
        ...
        return DataTables::of($result)
            ...
            ->editColumn('clock_in', function ($row) {
                ...
                if ($row->clock_in != null) {
                    ...
                    $input_value_clock_in = Carbon::createFromFormat('H:i:s', $row->timeIn, 'UTC')->timezone($this->timezones[admin()->company->timezone])->format('g:i A');
                }
                ...
                if ($row->clock_out != null) {
                    ...
                    $input_value_clock_out = Carbon::createFromFormat('H:i:s', $row->timeOut, 'UTC')->timezone($this->timezones[admin()->company->timezone])->format('g:i A');
                }
