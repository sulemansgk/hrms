...
class PayrollsController extends AdminBaseController
{
    ...
    public function check()
    {
        ...
        $startDate = Carbon::createFromDate(request()->get('year'), request()->get('month'), 1);
        $endDate = $startDate->copy()->endOfMonth();
        $period = CarbonPeriod::create($startDate, $endDate);
        $filteredPeriod = $period->filter(function ($date) {
            return $date->dayOfWeek !== Carbon::SUNDAY;
        });
        $numberOfDays = $filteredPeriod->count();

        $lateCounts = Attendance:: where('employee_id',request()->get('employee_id'))->whereMonth('date',request()->get('month'))
        ->whereYear('date',request()->get('year'))->where('is_late',1)->count();

  


        $this->data['lates'] = (int) floor($lateCounts/admin()->company->max_late_allowed ?? 3);
        ...
        $this->data['totalHoursClocked'] = Attendance::selectRaw('DATE_FORMAT(clock_in, "%Y-%m") AS month_year')
        ...
        $this->data['dayRate'] = ceil($this->basicSalary/$numberOfDays);
      
        $this->data['hourlyRate']= ceil($this->data['dayRate']/9);
        ...
        try {
            ...
            $this->hourly_rate = $this->data['hourlyRate'];
        }
