...
class Employee extends BaseModel implements AuthenticatableContract, AuthorizableContract, CanResetPasswordContract
{
    ...
    public function shift(){
        return $this->hasOne('App\Models\Shifts','id','shift_id');
    }
    ...
    public function scopeManager($query)
    {
        ...
        return $query->join('designation', 'designation.id', '=', 'employees.designation')->join('shifts','shifts.id', '=', 'employees.shift_id')
            ->join('department', 'designation.department_id', '=', 'department.id');
    ...
    public function MarkAttendance($r,$data){
        
        $lateAfterMin = $this->company->late_mark_after;
        $shift = $this->shift;
        $is_late = 0;

        $date = Carbon::createFromFormat("YmdHis", $data->io_time);

        $markedException = Carbon::createFromFormat('H:i:s',$shift->time_in)->addMinutes($lateAfterMin);

       

        if($date->format('his') > $markedException->format('his')){
           
            $is_late = 1;
        }

        //checking if the record is created or not if founf then update

        $attendance = Attendance::firstOrCreate(['employee_id' => $this->id, 'date' => $date->format('Y-m-d')]);

        if($attendance->wasRecentlyCreated){
            $attendance->status = 'present';
            $attendance->leaveType = null;
            $attendance->halfDayType = "no";
            $attendance->working_from = $r['headers']['dev_id'];
            $attendance->office_start_time = $shift->time_in;
            $attendance->office_end_time = $shift->time_out;
            $attendance->clock_in = $date->format('H:i:s');
            $attendance->is_late = $is_late;
            $attendance->notes = "Device Synced Data";
          

        }else{
            $attendance->clock_out = $date->format('H:i:s');
        }
        $attendance->save();
